    ...
    this.http.get<{content: any[]}>(COMPILED_URL).subscribe(response => {
      const data = response.content.map(item => {
        return {
          ...
          location: item?.answers?.['3']?.prettyFormat ? item?.answers?.['3']?.prettyFormat : (item?.answers?.['26']?.prettyFormat ? item?.answers?.['26']?.prettyFormat + " " + item?.answers?.['28']?.prettyFormat : "N/A"), // '3' is the key for the 'Location' question
